...
export default function ClientProofs() {
  ...
  async function load() {
    ...
    if (!clientId) { setProofs([]); setLoading(false); return }
    ...
    if (!itemIds.length) { setProofs([]); setClientMap(new Map()); setLoading(false); return }
    ...
    proofsData?.forEach(p => {
      ...
      if (proj?.client_id) map.set(proj.client_id, proj.clients?.company || proj.client_id)
    })
    setClientMap(map)
  ...
  if (searchQuery.trim()) {
    ...
    filtered = filtered.filter(p => {
      ...
      const ver     = p.version ? String.fromCharCode(64 + p.version) : String(p.version || '')
